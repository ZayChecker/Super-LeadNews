...
public class CacheService extends CachingConfigurerSupport {
    ...
    /*
      用户看过这篇文章
     */
    public Long userReadArticle(Integer userId, Long articleId){
        //set大key分片
        int hashVal = Math.abs(articleId.hashCode()) % 3;
        String key = ArticleConstants.ARTICLE_RESTRICT_TOPIC + ":" + userId.toString() + ":" + String.valueOf(hashVal);
        return stringRedisTemplate.opsForSet().add(key, articleId.toString());
    }

    /*
      set元素批量存进Redis
     */
    public void leadToRedisSet(String targetKey, Set<String> allArticleIds){
        List<Object> objects = stringRedisTemplate.executePipelined(new RedisCallback<Object>() {
            @Nullable
            @Override
            public Object doInRedis(RedisConnection redisConnection) throws DataAccessException {
                StringRedisConnection stringRedisConnection = (StringRedisConnection)redisConnection;
                String[] articleIdsArray = allArticleIds.toArray(new String[allArticleIds.size()]);
                stringRedisConnection.sAdd(targetKey, articleIdsArray);
                return null;
            }
        });
    }
